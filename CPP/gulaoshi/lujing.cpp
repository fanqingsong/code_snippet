...
#include <bits/stdc++.h>
using namespace std;
int n, q, to, az, ok, cnt, ans, mx[3], idx[3], x, y, b, f[1000006], mn[1000006], mne[1000006], nd[1000006],
    pre[1000006], suf[1000006];
...
void dfs1(int a, int fa, int c) {
    ...
    for (int i = 0; i < mp[a].size(); i++) {
        if (mp[a][i] != fa) {
            dfs1(mp[a][i], a, ((fa == 0) ? mp[a][i] : c));
            mn[a] = min(mn[a], mn[mp[a][i]]);
            mne[a] = min(mne[a], mn[mp[a][i]]);
        }
    }
    //	cout<<a<<":"<<mn[a]<<"==========---------"<<endl;
}
...
int main() {cnt = ans = 0, scanf("%d%d", &n, &q);
